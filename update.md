...
- **立案**: README.md の最初の `---` の直後・「はじめての方へ」見出しの前に `## ⬇️ ダウンロード` セクションを挿入。Mac（Apple Silicon）用 zip と Windows 用 exe のリンク、リリース一覧ページ、初回警告への補足注記を記載。既存本文・他セクションは変更しない。
- **影響範囲**: `README.md`（ダウンロードセクション追加のみ）、`update.md`（本エントリ追記）。
- **完了**: `README.md` に `## ⬇️ ダウンロード` セクションを挿入。detail.md の変更は不要（軽微な追記のため）。
...
- **ブランチ**: `feature/20260604-0152-readme-beginner-guide`。
- **進捗 — 完了**: README全面再構成。冒頭=かんたん使い方ガイド（用意するもの/起動/初回警告の回避/QR/指の使い方早見表/Mac権限/困ったとき/終了）、後半=作る人向け（ソース起動・ビルド・Tailscale・トラブル・構成）。実装変更なし。detail.md のREADME記述も更新。
