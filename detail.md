...
- 役割: ファイル依存とデータフロー（起動〜スマホ操作のシーケンス）をアニメーション付きインタラクティブSVGで可視化する単一HTMLファイル。外部CDN不使用・オフライン完全動作。
- 主要機能:
  - 自作 force simulation による依存グラフ（ノードドラッグ可、カテゴリ/エッジ種別カラーコード）
  - ホバーでハイライト、クリックで右サイドパネルにファイル詳細（役割/メソッド/依存/被参照）表示
  - 「動作の流れを再生」ボタンで flowSteps（9ステップ）をアニメ再生（粒子流動 + ノード点灯）
  - 再生/一時停止/前後ステップ/速度調整(0.5x/1x/2x)、ダークテーマ、日本語UI
- 依存: `detail.md` の情報を JS オブジェクトとして直接埋め込み（外部 fetch なし）。
- 被参照: なし（ローカル閲覧用 / `docs/` 配下）。
