...
_Key = None
_KEY_MAP = None
_ZOOM_MOD = None  # ズーム時の修飾キー（mac=Cmd / その他=Ctrl）


def _ensure():
    """pynput を初回だけ読み込み、Controller と各種定数を用意する。"""
    global _mouse, _keyboard, _Button, _Key, _KEY_MAP, _ZOOM_MOD
    ...
    # ズームの修飾キー: macOS は Cmd、Windows/Linux は Ctrl（多くのアプリで Cmd/Ctrl +/- が拡大縮小）
    _ZOOM_MOD = Key.cmd if platform.system() == "Darwin" else Key.ctrl
...
def handle_message(msg: dict) -> None:
    ...
    try:
        ...
        if mtype == "move":
            ...
            # 2本指ピンチによる拡大縮小。Cmd/Ctrl + '='(in) / '-'(out) を送る。
            key_char = "=" if msg.get("dir") == "in" else "-"
            with _keyboard.pressed(_ZOOM_MOD):
